...
def hillside_count_Pa(D,obj2k_func=lambda obj: int(obj)):
    #k, details = hillside_count(D)
    n = len(D[0])
    m = int(np.sum(D))
    ...
    for i in range(n):
        for j in range(n):
            x[i,j] = AP.addVar(ub=1,vtype="C",name="x(%s,%s)"%(i,j)) #continuous
    ...
    for i in range(n):
        for j in range(n):
            for k in range(n):
                if j != i and k != j and k != i:
                    I.append((i,j,k))
    #for idx in details["model"]._constraints:
    ...
    details = {"x": get_sol_x_by_x(x,n)(),"c":c}
